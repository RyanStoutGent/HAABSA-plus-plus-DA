...
from att_layer import dot_produce_attention_layer, bilinear_attention_layer, mlp_attention_layer, Mlp_attention_layer, \
    cam_mlp_attention_layer, mlp_layer, triple_attention_layer
from config import *
from utils import load_w2v, batch_index, load_inputs_cabasc


def cabasc(input_fw, input_bw, sen_len_fw, sen_len_bw, target, sen_len_tr, ave_sent, mult_mask, sent_full, keep_prob1,
           keep_prob2, _id='all'):
    print('I am cabasc.')
    ...
    att_weights = triple_attention_layer(M, target_3d, ave_sent_3d, sen_len_fw + sen_len_bw - sen_len_tr,
                                         FLAGS.n_hidden, FLAGS.l2_reg, FLAGS.random_base, 'eq7')  # batch_size x 1 x N
...
def main(train_path, test_path, accuracyOnt, test_size, remaining_size, learning_rate=FLAGS.learning_rate,
    ...
    with tf.device('/gpu:1'):
        ...
        ave_sent = tf.divide(tf.reduce_sum(sentence, 1), tf.reshape(
            tf.tile(tf.cast(sen_len_bw + sen_len - tar_len, tf.float32), [FLAGS.embedding_dim]),
        ...
        target = tf.divide(tf.reduce_sum(target, 1),
                           tf.reshape(tf.tile(tf.cast(tar_len, tf.float32), [FLAGS.embedding_dim]),
                                      [-1, FLAGS.embedding_dim]))
        ...
        prob, alpha_fw, alpha_bw, alpha_t_l, alpha_t_r = cabasc(inputs_fw, inputs_bw, sen_len, sen_len_bw, target,
                                                                tar_len, ave_sent, mult_mask, sent_full, keep_prob1,
                                                                keep_prob2, 'all')
    ...
    with tf.Session(config=config) as sess:
        ...
        def get_batch_data(x_f, sen_len_f, x_b, sen_len_b, yi, target, tl, batch_size, kp1, kp2, senshort, sen,
                           multMask, is_shuffle=True):
        ...
        for i in range(FLAGS.n_iter):
            ...
            for train, numtrain in get_batch_data(tr_x, tr_sen_len, tr_x_bw, tr_sen_len_bw, tr_y, tr_target_word,
                                                  tr_tar_len,
                                                  FLAGS.batch_size, keep_prob, keep_prob, tr_sen_short, tr_sent,
                                                  tr_mult_mask):
            ...
            for test, num in get_batch_data(te_x, te_sen_len, te_x_bw, te_sen_len_bw, te_y,
                                            te_target_word, te_tar_len, 2000, 1.0, 1.0, te_sen_short, te_sent,
                                            te_mult_mask, False):
                if FLAGS.method == 'TD-ATT' or FLAGS.method == 'IAN':
                    ...
                    _loss, _acc, _ty, _py, _p, _fw = sess.run([loss, acc_num, true_y, pred_y, prob, alpha_fw],
                                                              feed_dict=test)
                ty = np.asarray(_ty)
                py = np.asarray(_py)
                fw = np.asarray(_fw)
